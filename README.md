...
Players choose one of the three active quests in `/quests`. Only the selected quest gains progress. After claiming its reward, the player must wait five minutes before selecting another quest. Change this using `selection-cooldown` in `config.yml`.
